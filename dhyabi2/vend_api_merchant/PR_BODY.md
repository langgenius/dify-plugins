...
- Package: `vend_api_merchant.difypkg` (in this PR, under `dhyabi2/vend_api_merchant/`)
- Source repo: `https://github.com/dhyabi2/vend` (branch `etch/work`,
...
- **SDK pin**: `requirements.txt` pins `dify_plugin>=0.5.0`, meeting the current
  Marketplace reviewer bar (check #10). Repackaged 2026-09-21 from the earlier
  build that used the older pluggable `dify-plugin-sdk` pin.
- **Network requests**: the plugin makes HTTPS requests only to the configurable
  `vend_base_url` (default `extract.paypercall.dev`) with the user-supplied
  `url`/`q` parameter. The parameter is URL-encoded; the plugin issues
  a standard `httpx.get` with a 30 s timeout. No arbitrary host is contacted.
- **No secrets stored**: the only provider credential is the user's own
  `payment_header` (their Nano block hash) used to authenticate a paid call; it is
  stored by Dify's own credential vault and is not logged or transmitted anywhere
  except the configured Vend endpoint.
- **No code execution, no data persistence**: the plugin runs no local code
  execution, opens no files, and does not read or write any data beyond the HTTP
  request/response for the call. It stores nothing on the Dify host.
- **Payment disclosure**: the plugin does not move funds itself. It calls a
  pay-per-call API that answers HTTP 402 with an x402 v2 challenge; the user
  settles the Nano payment in their own wallet. This is fully disclosed in the
  Folder README and PRIVACY.md per Dify's financial-activity guideline.
- **No third-party data sharing, no analytics, no tracking** (see PRIVACY.md).
...
- Disclosure: this is a ready, fully-functioning integration a Dify user can install
  today. Vend API Merchant is a pay-per-call API; there is no API key or signup to
  onboard — the Nano wallet is the account.
