...
def _process_single_image(
    ...
    payload = {**payload_common, **input_data}
...
class BatchProcessImagesTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if len(inputs) > MAX_BATCH:
            yield from yield_error(
                self, f"批量处理最多支持 {MAX_BATCH} 张图片，当前提供了 {len(inputs)} 张"
            )
            return

        if not inputs:
            yield from yield_error(
                self, "请通过 image_urls 或 image_base64s 至少提供一张图片"
            )
            return
        ...
        yield self.create_text_message(
            f"批量处理完成：{success_count}/{len(inputs)} 张图片处理成功。"
            "Alt 文本应由 Dify LLM 节点生成。"
        )
