    ...
    def _invoke(
        ...
        if audio_url:
            try:
                ...
                yield self.create_text_message(f"Generated SFX URL is ready, but downloading it failed: {exc}")
            else:
                mime = guess_mime_type(output_format, content_type)
                yield self.create_blob_message(blob=content, meta={"mime_type": mime, "filename": "sonilo_sfx_video"})
